<x-admin::layouts>
    ...
    <div class="flex items-center justify-between">
        <p class="text-xl font-bold text-gray-800 dark:text-white">
            @lang(($meta['badgeLabel'] ?? ucfirst($type)))
        </p>
        ...
        <div class="flex items-center gap-x-2.5">
            ...
            >
                @lang('Create ' . ucfirst($meta['itemNounSingular'] ?? rtrim($type, 's')))
            </a>
        </div>
    </div>

    {!! view_render_event('bagisto.admin.' . $type . '.list.before') !!}

    <!-- Filters -->
    <form method="GET" action="{{ route('admin.' . $type . '.index') }}" class="mt-5 flex flex-wrap items-end gap-2.5">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400">{{ $meta['eraLabel'] ?? 'Era' }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400">{{ $meta['regionLabel'] ?? 'Region' }}</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <a
                                            href="{{ route('admin.' . $type . '.edit', $item->id) }}"
    ...
    {!! view_render_event('bagisto.admin.' . $type . '.list.after') !!}
</x-admin::layouts>
